odoo.define('atharva_theme_base.s_product_var_slider_options',function(require){
'use strict';
...
ajax.loadXML('/atharva_theme_base/static/src/xml/product_variant_slider_popup.xml', core.qweb);
...
options.registry['product_variant_slider_actions'] = options.Class.extend({
    ...
    product_variant_slider_configure: function(previewMode, value){
        ...
        if(previewMode === false || previewMode === "click"){
            self.$modal = $(qweb.render("atharva_theme_base.p_var_slider_popup_template"));
            $('body > #product_variant_slider_modal').remove();
            ...
            self._rpc({
                ...
                var pro_col_ele = $("#product_variant_slider_modal select[name='pro_collection']");
                pro_col_ele.empty();
                ...
                if(data){
                    var pro_tab_ele = $("#product_variant_dynamic_slider_modal select[name='pro_collection']");
                    pro_tab_ele.empty();
                    for(var i = 0; i < data.length; i++){
                ...
                $('#pro_all').on('click', function(e){
                  e.preventDefault();
                  $('#multiselect option').prop('selected', true).trigger('chosen:updated');
                });

                $('#pro_reset').on('click', function(e){
                  e.preventDefault();
                  $("#multiselect option:selected").removeAttr("selected").trigger('chosen:updated');
                });
                ...
                self._rpc({
                    ...
                    var slider_design_ele = $("#product_variant_slider_modal select[name='slider_type']");
            ...
            $('#product_variant_slider_modal').on('shown.bs.modal', function (e) {
                if(sn_play != false){
                    $("div.slider_time").show();
